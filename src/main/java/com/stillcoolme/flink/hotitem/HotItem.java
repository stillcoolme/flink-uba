...
import com.stillcoolme.flink.hotitem.entity.ItemViewCount;
...
*/
        ...
        DataStream<ItemViewCount> windowAggStram = dataStream
                .filter(data -> "pv".equals(data.getBehavior()))    // 过滤pv行为
                .keyBy("itemId")    // 按商品ID分组
                .timeWindow(Time.hours(1), Time.minutes(5))    // 开滑窗
                .aggregate(new ItemCountAgg(), new WindowItemCountResult());

        // ItemCountAgg 窗口聚合规则，增量聚合函数，来一个聚合一个
        // WindowItemCountResult 定义输出数据结构，用来拿window信息，全窗口函数，，本来是拿当前窗口全部数据的，但在这里拿前一个函数的聚合结果，包装成输出结果

        // 5. 收集同一窗口的所有商品count数据，排序输出top n
        DataStream<String> resultStream = windowAggStram.keyBy("windowEnd")
    ...
    public static class WindowItemCountResult implements WindowFunction<Long, ItemViewCount, Tuple, TimeWindow> {
        @Override
        public void apply(Tuple tuple, TimeWindow window, Iterable<Long> input, Collector<ItemViewCount> collector) throws Exception {
